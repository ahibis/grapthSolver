...
export function dfs<T, Y>(data: T[], options: dfsOption<T,Y>, payload?: Y) {
  const { getHash, getChildrens, isFinished, checkChildren, getScore, compareNode } =
    options;
  let queue: withParent<T>[] = data.map((d) => ({
    data: d,
    parent: undefined,
    depth: 0,
  }));
  ...
  while (queue.length > 0) {
    ...
    if (getScore) {
      queue.sort(
        (a, b) => getScore(b.data, b.depth, payload) - getScore(a.data, a.depth, payload)
      );
    }
  }
  return [];
